...
| Current/all page export | EXPORT | Debug/Release pass (0 warnings, 0 errors) | 106 passed, 0 failed | GUI手動操作はNot verified。STA renderer integrationはverified | PNG/JPEGの論理寸法、透明/白背景を検証 | active page復元はMainWindow実装、serviceはpage単位継続 | 日本語dialog/error/summary | 既存project形式変更なし | TASK-050 / report参照 | 順序、zero padding、日本語/無効文字/予約名、衝突、無効出力先を検証 |
