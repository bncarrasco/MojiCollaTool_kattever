...
| Auto layout | LAYOUT | Debug/Release pass (0 warnings, 0 errors) | Debug/Release 195 passed, 0 failed | Manual WPF pointer/DPI/final-pixel inspection Not verified | Grapheme-aware horizontal/vertical basic wrap、CRLF/LF preservation、surrogate/combining/variation/ZWJ/RI safety、minimum font and overflow warning | FullText unchanged; FitTextToBalloon changes text visual/font/position only; FitBalloonToText changes balloon bounds/position only; mode and alignment round-trip | Japanese explicit Apply UI、Start/Center/End mapping、locked/hidden/invalid safe state | TextLayoutService bounded cache、visual-only plan、one-way mode、format 2.2 fields unchanged | TASK-140 / local implementation | TASK-140AutoTextLayoutTests 3 tests: Unicode/basic wrap、vertical deterministic minimum-font result、one-history FitTextToBalloon Undo/Redo |
