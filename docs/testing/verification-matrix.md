...
| Auto layout | LAYOUT | Debug/Release build/test pass (0 warnings, 0 errors) | Debug/Release 208 passed, 0 failed | 実WPF pointer/DPI/IME/final-pixel目視は未検証 | 書記素単位の基本wrap、明示改行LF/CR/CRLF/mixed保持、surrogate/combining/variation/ZWJ/RI安全、最小文字サイズとoverflow警告 | FullText不変。FitTextToBalloonは文字のvisual/font/positionのみ、FitBalloonToTextは枠のbounds/positionのみを変更し、mode/alignmentをround-trip | 日本語の実Apply Button／ComboBox、Start/Center/End、lock/visibility対象、redo branch、visual hierarchy、AttachedSymbol | TextLayoutServiceのbounded cache、visual-only plan、明示適用mode、format 2.2 fields維持、mode別padding、深いrollback | TASK-140 / `5a74db3a8c38d7b2b900d11109942c4460d7e3de` | TASK140AutoTextLayoutTests 16 tests、実PageEditor経路24 composition×両mode性能（Release p95 36.124 ms、FitText24 755.202 ms、FitBalloon24 715.984 ms） |
...
- Debug/Release 208/208合格、警告0・エラー0。実WPF pointer/DPI/IME/final-pixel目視は未検証です。
