...
| Basic balloon model/rendering | BALLOON | Not run: .NET SDK 6.0.428 unavailable | Geometry tests added; not run | Not verified: manual WPF interaction | Tategaki text relationship round-trip | Yokogaki/Unicode text relationship round-trip | Japanese shape selector/add command added | page.xml version 2.1, version 2.0 read compatibility, future version rejection, unknown shape fallback, invalid numeric validation; geometry fallback/cache covered | TASK-110/TASK-120 | T120 rendering/handles implemented; build/test blocked by missing SDK |
