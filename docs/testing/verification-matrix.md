...
| Z-order/lock | ZORDER/LOCK | Debug/Release pass、警告0・エラー0 | Debug/Release 238/238、TASK-080専用26/26 | 実OS pointer/DPI/IME/final-pixel/context menuとproduction routed right-click選択はNot verified | 3型共通4 Z操作、routed left-clickの日本語拒否／captureなし、gesture途中lock取消、MojiWindow即時反映、selection helper＋context actionによるlocked解除を確認 | linked／unlinked typed block、callback単回評価、検査済みcandidateとTail/TextLinkのdeep-clone alias隔離、format 2.3日本語path round-trip | toolbar／context／status／拒否理由は日本語 | 2.0〜2.2は既存suiteで間接確認。2.3保存、canonical Z、relationship／text削除lock、validation／callback例外、coalesce／Undo/Redo／dirty、context handler解放を直接確認 | TASK-080 integrated `3b2e241ed79e179621139c50f1739f50a96b6886`; 司令役介入 `c10942c279d6c28f1b9eb18a6f081676736c5942`; test `d5a1b03c055427eea2fb2677bdc4fb48233b00bd` | C080-01〜17。3型×4操作×toolbar/contextでoperation別順序、model/live/Canvas/selectionを確認。通常review 4巡、介入1回、介入後review 1巡。性能p95 1.186/1.435 ms、100操作103.356/116.178 ms |
...
| Non-destructive balloon merge | BALLOON-MERGE/ZORDER/LOCK/UNDO | Debug/Release pass、警告0・エラー0 | Debug/Release 254/254、TASK-150専用16/16 | 実WPF pointer/DPI/IME/final-pixel目視はNot verified | linked Tategaki text、layout state、付加記号を保持してgroup移動・取消・Undo/Redoを確認 | linked Yokogaki text、unlinked member、全member tailを同一deltaで移動し、member dataをmerge/unmerge前後でdeep比較 | 実XAMLの候補ComboBox、merge/unmerge、合体中layout Apply拒否、日本語status、handle無効化、merge同期失敗rollbackを確認 | version 2.4 ordered merge round-trip、2.0〜2.3 mergeなし、2.5拒否、invalid 2.4 atomic拒否、日本語project/page/path | TASK-150 / `8aa9a7431891700821b153a7d074ca7230e505b0` | 5 shape、rotation、overlap/touch/containment/non-overlap、全tail＋primary style、primary visibility、内部stroke抑制、transparent gap exact hit、各member×4実Z＋Canvas、lock deletion、overflow／late failure atomic move、Undo/Redo rebind、remove／Disposeを確認。Debug p95 0.045 ms／100 batch 79.505 ms、Release p95 0.039 ms／100 batch 62.594 ms |
...
- merge dataのstrict validation、flat group combine、clone ID preserve/remap、primary削除、cross-group重複拒否を確認しました。
- memberのshape／position／size／rotation／style／tail／link／layoutをmerge/unmerge前後でdeep比較し、typed composition、4 Z操作基盤、全member lock preflight、履歴・dirty・redo branch・subscriber例外境界を確認しました。
- 実XAML UI、one merge visual、original visual抑制、exact geometry hit、個別resize/tail handle抑制、group gesture preview/commit/cancel/mid-lock、page rebind/unbindをSTAで確認しました。
- version 2.4 round-trip、2.3 mergeなし読込、2.5拒否、invalid groupのcommit前拒否とarchive／working data保護、日本語project/page/pathを確認しました。
- review修正後は、各memberからの4実Z buttonとCanvas順、全tailの同時描画とprimary style／visibility、unlinked member移動、合体中layout Apply拒否、model delete lock、overflow／late symbol failure、merge同期失敗rollback、Undo/Redo rebind、remove、Dispose handler解放をproduction経路で追加確認しました。
- Debug/Release 254/254、TASK-150専用16/16合格、警告0・エラー0です。実WPF pointer/DPI/IME/final-pixel目視はNot verifiedです。
