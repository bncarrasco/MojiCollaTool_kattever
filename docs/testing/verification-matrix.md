...
| Z-order/lock | ZORDER/LOCK | Debug/Release pass; Release 0 warnings/0 errors | Debug/Release 79 passed, 0 failed | Not verified | Not verified | Not verified | Required by TASK-080 | UUID/Type/ZIndex/lock/visibility/relationship fields; canonical page order, clone remap, legacy XML defaults, and versioned round-trip covered | TASK-060 / `8a0e83b` | UI operations remain TASK-080 |
...
| Fork identity | FORK | Debug/Release pass (0 warnings, 0 errors) | 73 passed | MainWindow title、ヘルプ→このアプリについて、Aboutの非公式フォーク・原作者非関係・問い合わせ注意、AssemblyTitle/package metadataを確認 | LICENSE/THIRD-PARTY-NOTICES.mdをDebug/Release/publishへ同梱 | 4 ColorSelector filesにWpfColorPicker/Apache-2.0由来・local changes通知を確認 | AboutからLICENSE/THIRD-PARTY-NOTICES.mdを案内 | Root MIT retained; copyright `2024 kuramiya` retained; Company/Authorsの製品名設定を削除 | TASK-190B | publish `artifacts/publish/Release`で2ライセンスファイル存在確認。installer/ZIP/manual app launchはscope外/Not verified |
