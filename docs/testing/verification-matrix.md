...
| Basic balloon model/rendering | BALLOON | Debug/Release pass (0 warnings, 0 errors) | Debug/Release 190 passed, 0 failed | Manual WPF pointer/DPI/final-pixel inspection Not verified | Tategaki link preserves direction/font/content | Yokogaki/Unicode link preserves direction/font/content | Japanese shape/tail/link/Z-order controls and safe error state | page.xml version 2.2、2.0/2.1 read、future version rejection、geometry fallback/cache、2.2 duplicate-link migration | TASK-110/TASK-120/TASK-130（C130修正commitは完了報告） | body/tail geometry hit、3 tail handle、composition move/Z、history、legacy duplicate migrationを実装 |
| Tail/text typed composition | BALLOON/ZORDER/UNDO | Debug/Release pass (0 warnings, 0 errors) | TASK-130専用15 passed; suite 190 passed, 0 failed | 実マウスpointer/DPI/final-pixel目視 Not verified | Tategaki text remains unchanged on link/move | Yokogaki/Unicode link remains unchanged on link/move | 実ComboBox＋link/unlink button、4実Z button、Canvas typed順、端no-op、saved dirty／Undo/Redo／redo branch、page lifecycle／Dispose、hidden handleをSTAで確認 | single-tail、同一page一対一link、text削除unlink、balloon削除text保持、typed block保存再読込、2.2 duplicate migration、2.0/2.1互換 | TASK-130 / C130修正を含む最終commit（完了報告にfull hash） | Debug/Release性能: tail 598.761/424.342 ms、root 0.009/0.224 ms、composition block 83.906/84.613 ms（各3,000 ms閾値、詳細はreport） |
