...
| Z-order/lock | ZORDER/LOCK | Not verified: .NET SDK 6.0.428 missing（dotnet test exit 1） | 8 identity/Z-order test methods present; not executed because SDK unavailable | Not verified | Not verified | Not verified | Required by TASK-080 | UUID/Type/ZIndex/lock/visibility/relationship fields; canonical page order, clone remap, legacy XML defaults, and versioned round-trip covered | TASK-060 / `6ff780d` | UI operations remain TASK-080 |
