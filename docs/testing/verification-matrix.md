...
| Attached symbols | SYMBOL | Debug/Release pass (0 warnings, 0 errors) | Debug/Release 175 passed, 0 failed | WPF manual pointer/DPI/IME/final-pixel inspection Not verified | parent/anchor/offset/inheritance/decoration/spacing/detached round-trip | SetMojiDatas、orphan policy、cross-object ID atomicity、Undo/Redo、2.1 read compatibility、UI capture/reanchor/selection、detached save/reload、hidden rendering、実描画mixed Z-order、grapheme refresh baseline、24-symbol refresh/drag measurement、ScrollViewer viewport、UI invalid-input rejection、ParentId別UI一覧、追加直後ZIndex、親回転local offset/drag、同一行再クリック、Expander折りたたみ | Required by TASK-100 | page format 2.2, 2.0/2.1 reader compatibility, future 2.3 rejection | TASK-090/TASK-100 | TASK100AttachedSymbolUiTests 20 tests: grapheme edge cases, all candidates/arbitrary text, two recognized fonts and both directions, placement/parent transform, font fallback/name, decoration/spacing, mutually-exclusive selection, Japanese UI boundary rejection, ScrollViewer measure/arrange and collapse, ParentId-filtered UI CRUD with Undo/Redo, immediate rendered canonical Z-order with Undo/Redo, parent-rotation offset/drag numeric coverage, same-row text re-selection, detached preservation, IsVisible=false, rendered mixed canonical Z-order with save/reload, refresh/drag performance |
| Basic balloon model/rendering | BALLOON | Debug/Release pass (0 warnings, 0 errors) | Debug/Release 190 passed, 0 failed | Manual WPF pointer/DPI/final-pixel inspection Not verified | Tategaki link preserves direction/font/content | Yokogaki/Unicode link preserves direction/font/content | Japanese shape/tail/link/Z-order controls and safe error state | page.xml version 2.2、2.0/2.1 read、future version rejection、geometry fallback/cache、2.2 duplicate-link migration | TASK-110/TASK-120/TASK-130 (`a76d26671fe789314d4da1c63a58c7e52bf168d0`) | body/tail geometry hit、3 tail handle、composition move/Z、history、legacy duplicate migrationを実装 |
| Tail/text typed composition | BALLOON/ZORDER/UNDO | Debug/Release pass (0 warnings, 0 errors) | TASK-130専用15 passed; suite 190 passed, 0 failed | 実マウスpointer/DPI/final-pixel目視 Not verified | Tategaki text remains unchanged on link/move | Yokogaki/Unicode link remains unchanged on link/move | 実ComboBox＋link/unlink button、4実Z button、Canvas typed順、端no-op、saved dirty／Undo/Redo／redo branch、page lifecycle／Dispose、hidden handleをSTAで確認 | single-tail、同一page一対一link、text削除unlink、balloon削除text保持、typed block保存再読込、2.2 duplicate migration、2.0/2.1互換 | TASK-130 / `a76d26671fe789314d4da1c63a58c7e52bf168d0` | Debug/Release性能: tail 598.761/424.342 ms、root 0.009/0.224 ms、composition block 83.906/84.613 ms（各3,000 ms閾値、詳細はreport） |
