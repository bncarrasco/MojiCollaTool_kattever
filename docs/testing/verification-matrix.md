...
| Auto layout | LAYOUT | Debug/Release build/test pass (0 warnings, 0 errors) | Debug/Release 209 passed, 0 failed | 実WPF pointer/DPI/IME/final-pixel目視は未検証 | 書記素単位の基本wrap、明示改行LF/CR/CRLF/mixed保持、surrogate/combining/variation/ZWJ/RI安全、最小文字サイズとoverflow警告 | FullText不変。FitTextToBalloonは文字のvisual/font/positionのみ、FitBalloonToTextは枠のbounds/positionのみを変更し、mode/alignmentをround-trip。frame resizeはUnapplied化、move／tailはplan維持 | 日本語の実Apply Button／ComboBox、Start/Center/End、lock/visibility対象、redo branch、visual hierarchy、AttachedSymbol | TextLayoutServiceのbounded cache、visual-only plan、明示適用mode、format 2.2 fields維持、mode別padding、深いrollback。C140-15の2.3移行は承認待ち | TASK-140 / `a54177d527f79e0ef06556c1ac81c39a5a9d5044` | TASK140AutoTextLayoutTests 17 tests、実PageEditor経路24 composition×両mode性能（Release p95 36.124 ms、FitText24 755.202 ms、FitBalloon24 715.984 ms） |
...
- Debug/Release 209/209合格、警告0・エラー0。実WPF pointer/DPI/IME/final-pixel目視は未検証です。C140-15のversion 2.3移行は承認待ちです。
