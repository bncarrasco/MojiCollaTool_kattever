...
- **Decision:** `FitTextToBalloon`と`FitBalloonToText`に加え、未適用を表す`Unapplied`を採用する。1回のlayoutで変更可能な側を一方に限定し、適用stateはversion 2.3で正式保存する。version 2.0〜2.2のlinkは読込時に全て`Unapplied`へ移行し、2.4以降は拒否する。
