...
- **Composition:** merge memberごとの既存`balloon + linked text + non-detached attached symbols`を結合し、一つのtyped Z-order blockとして扱う。merge group moveは全member compositionを同じdeltaで移動する。member bodyの個別resizeは解除後に行う。TASK-150時点ではtail handle編集も無効だが、TASK-160導入後はmerge完了状態を維持したまま各memberの各tailを個別選択・編集できる。
