...
- **Geometry:** member bodyをpage座標へ変換してunionし、body fillと外周strokeを一度だけ描画する。TASK-150では各memberのsingle tailを保持して合成表示し、TASK-160導入後は全memberの全tailを表示する。non-overlapは複数の離れた領域として正常状態とする。geometry/cache keyはordered member IDとshape／bounds／position／rotation／tail collectionを含む。
- **Composition:** merge memberごとの既存`balloon + linked text + non-detached attached symbols`を結合し、一つのtyped Z-order blockとして扱う。merge group moveは全member compositionを同じdeltaで移動する。TASK-160導入後、body resizeはmerge group全体に対する単一操作だけを許可し、member bodyの個別resizeは許可しない。unmerge後は各member bodyを個別resizeできる。TASK-150時点ではtail handle編集も無効だが、TASK-160導入後はmerge完了状態を維持したまま各memberの各tailを個別選択・編集できる。
- **Lock/atomicity:** merge、unmerge、group move、group resize、group Z-orderは全member compositionをpreflightし、locked／missing／duplicate／invalid relationshipがあればmodel、live visual、selection、history、dirty、status notificationを部分変更せず拒否する。成功は各semantic操作につき1 Undo entryとする。
...
- **Consequences:** 2.4で保存したprojectは2.3以前のreaderでは開けない。旧project読込と元fileを保護するatomic writerは維持される。TASK-160はtail collectionを追加してもmerge relationshipを維持し、tail semantic editごとにmerge visualを再構築する。TASK-230はselection UIを再利用できるが、merge dataやtyped compositionを一般`GroupId`へ暗黙変換しない。
