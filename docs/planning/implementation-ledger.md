...
| TASK-050 | Dependency ready | `feature/TASK-050-batch-page-export` | 未作成 | `7fc27d5`（T030統合HEAD） | - | EXPORT | - | Not run | Not run | Not verified | Worker prompt/worktree準備待ち。MainWindow競合のためT190B完了後に開始 | なし | TASK-060との並行候補、T190B完了後 |
| TASK-060 | Dependency ready | `feature/TASK-060-object-id-zindex` | 未作成 | `7fc27d5`（現develop） | - | OBJECT,ZORDER | ADR-0003 | Not run | Not run | Not verified | Worker prompt/worktree準備待ち。MainWindow変更禁止でUI laneと分離 | なし | 最優先の並行基盤task |
...
| TASK-190B | Complete | `feature/TASK-190B-app-branding` | `F:/github/MojiCollaTool-worktrees/TASK-190B` | `1e64a71`（develop） | 修正後commit参照 | FORK,UI,DEPS | - | Debug/Release pass (0 warnings, 0 errors) | 73 passed | publish pass、Debug/Release/publishへLICENSE類同梱確認。AssemblyTitle、Company/Authors、4 ColorSelector noticesを確認 | Pending integration review | TASK-200。T060と並列可能、T050は本task完了後 |
