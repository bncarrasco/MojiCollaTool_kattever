...
| コマンド | 結果 |
| --- | --- |
| `powershell.exe -ExecutionPolicy Bypass -File eng\test.ps1` | 成功: 38件成功、0件失敗 |
...
日本語のプロジェクト名・ページ名・本文とページごとの背景画像（0枚・1枚・2枚）の往復、日本語パス、画像の不要entry不在、未知のmajor版、将来のminimum reader、DTD、Zipパス逸脱、ページ順序不連続、ページ名不一致、画像source失敗時の原本保護、1世代バックアップ置換を検証しました。
