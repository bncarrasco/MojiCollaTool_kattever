...
- MSTest test project、Debug/Release/test script、現行mctzip root fixture、XML round-trip characterization testを追加した。
...
SDK準備後にMSTestをtest-only依存として追加した。既存公開interfaceを変えず、ProjectReferenceでproduct assemblyを対象にXML serializerの観測値を固定した。test build生成物を追跡しないため、`tests/.gitignore`に`bin/obj`を追加した。新規ADR候補はない。
...
| `powershell -File eng/test.ps1` | **Pass** (exit 0) | 5 passed, 0 failed, 0 skipped |
...
- XML/zip互換性、日本語path、fixture round-trip: **Verified within current format** — XML round-trip、root entry、mctzip archive entry、Unicodeを自動確認。公式版との互換性は保証しない。
- `bin/obj`除外: product側設定に加え、新規test projectの生成物を除外する`tests/.gitignore`を追加した。

## Known risks and follow-up

- REQ-NFR-BUILD-001とREQ-NFR-TEST-001のローカル受入条件は満たした。REQ-NFR-PERF-001の数値性能計測は未実施であり、性能退行を保証しない。
